...
app.get('/', (req, res) => {
  res.json({
    ...
    docs: 'https://github.com/iotdb/iotdb',
  });
});
...
async function start() {
  ...
  const server = app.listen(PORT, () => {
    console.log(`[iotdb] Server running on http://localhost:${PORT}`);
    console.log(`[iotdb] Backend: ${BACKEND_NAME}`);
    console.log(`[iotdb] Data file: ${DATA_FILE}`);
  });

  // Graceful shutdown
  const shutdown = async (signal) => {
    console.log(`\n[iotdb] Received ${signal}, shutting down...`);
    server.close(async () => {
      await backend.close();
      console.log('[iotdb] Server stopped');
      process.exit(0);
    });

    // Force exit after 5 seconds
    setTimeout(() => {
      console.error('[iotdb] Forced shutdown after timeout');
      process.exit(1);
    }, 5000);
  };
...
start().catch((e) => {
  console.error(`[iotdb] Failed to start: ${e.message}`);
  process.exit(1);
});
