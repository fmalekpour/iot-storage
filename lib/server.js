...
const Router = require('./backends/router');
...
app.get('/data/*', async (req, res) => {
  try {
    ...
    if (rows.length === 0) {
      // No record at this path — try treating the last segment as a field name
      const lastSlash = recordPath.lastIndexOf('/');
      if (lastSlash > 0) {
        const parentPath = recordPath.slice(0, lastSlash);
        const fieldName = recordPath.slice(lastSlash + 1);

        // Reserved/system fields can't be accessed this way
        if (!fieldName || fieldName.startsWith('_')) {
          return res.status(404).json({ error: `No records found for path: ${recordPath}` });
        }

        const parentRow = await backend.getExact(parentPath);
        if (parentRow && fieldName in parentRow) {
          const value = parentRow[fieldName];
          // Send strings as plain text to avoid JSON quote wrapping
          // (curl users see unwrapped strings; scripts still get valid content)
