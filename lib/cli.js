...
program
  .command('start')
  .description('Start iotdb as a background daemon')
  ...
  .action((opts) => {
    ...
    try {
      ...
      console.log(`✓ iotdb started (PID ${pid}) on port ${config.get('port')}`);
...
program
  .command('stop')
  .description('Stop the iotdb daemon')
  .action(async () => {
...
program
  .command('restart')
  .description('Restart the iotdb daemon')
  ...
  .action(async (opts) => {
    ...
    try {
      // Stop if running
      if (daemon.isRunning()) {
        process.stdout.write('Stopping iotdb... ');
        await daemon.stop();
        console.log('done');
      }
      ...
      // Start
      const pid = daemon.start(path.join(__dirname, 'server.js'), opts);
      console.log(`✓ iotdb started (PID ${pid}) on port ${cfg.get('port')}`);
    } catch (e) {
      console.error(`✗ ${e.message}`);
      process.exit(1);
    }
...
program
  .command('run')
  .description('Run iotdb in the foreground (debug mode)')
  .option('-p, --port <number>', 'Port to listen on (default: 9123)')
  .option('-d, --data <path>', 'Path to the JSON data file')
  .option('-b, --backend <name>', 'Storage backend (default: json)')
  .option('-c, --config <path>', 'Path to config file')
  .action((opts) => {
    ...
    process.env.IOTDB_PORT = String(config.get('port'));
    process.env.IOTDB_DATA_FILE = config.get('dataFile');
    process.env.IOTDB_BACKEND = config.get('backend');

    console.log(`iotdb v${pkg.version}`);
...
program
  .command('status')
  .description('Check if iotdb daemon is running')
  .action(() => {
    const cfg = new Config();
    cfg.load();
    ...
    if (pid && pidManager.isAlive(pid)) {
      console.log(`✓ iotdb is running (PID ${pid})`);
    } else {
      if (pidManager.exists()) {
        pidManager.cleanup();
      }
      console.log('✗ iotdb is not running');
    }
  });
