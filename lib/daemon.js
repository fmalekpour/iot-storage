...
class DaemonManager {
  ...
  start(serverScript, cliArgs = {}) {
    ...
    if (this.pidManager.isAlive()) {
      ...
      throw new Error(`iotdb is already running (PID ${pid})`);
    }

    // Build environment for the child — pass config via env vars
    const childEnv = { ...process.env, IOTDB_DAEMON: '1' };
    if (cliArgs.port) childEnv.IOTDB_PORT = String(cliArgs.port);
    if (cliArgs.data) childEnv.IOTDB_DATA_FILE = cliArgs.data;
    if (cliArgs.backend) childEnv.IOTDB_BACKEND = cliArgs.backend;
  ...
  async stop() {
    const pid = this.pidManager.read();
    if (!pid) {
      throw new Error('iotdb is not running (no PID file)');
    }
    if (!this.pidManager.isAlive(pid)) {
      this.pidManager.remove();
      throw new Error('iotdb PID file existed but process was already dead — cleaned up');
    }
